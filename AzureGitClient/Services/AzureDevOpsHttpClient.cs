...
namespace AzureGitClient.Services
{
    public sealed class AzureDevOpsHttpClient : IAzureDevOpsHttpClient
    {
        ...
        public string getIdOfSpecificRepoFile(string filePath)
        {
            string fileId = null;
            string fileNotFound = "No File Was Found With Associated File Path:";
            List<SHA1ItemIdValue> items = _sha1Values.Value.Where(x => x.ObjectId != null).ToList();
            if (items.Count > 0)
            {
        ...
        public ProcessResponse getListOfIdsOfRepoFilesByRepo(string repoName)
        {
        ...
        public string pushEditBranchChanges(string branchName, string content, string pushURL, string commitComment)
        {
            string result = "";
            VssBasicCredential credentials = new VssBasicCredential(string.Empty, _configuration["PAT"]);
            VssConnection connection = new VssConnection(new Uri(string.Format("https://dev.azure.com/{0}", "PlaceHolder")), credentials);
            using (GitHttpClient gitClient = connection.GetClient<GitHttpClient>())
            {
                ...
                try
                {
                    baseBranch = gitClient.GetBranchAsync(repo.ProjectReference.Id, repo.Id, branchName).Result; //Try getting the specified branch for the repo
                }
                catch (AggregateException ae)
                {
                    if (ae.InnerException == null || ae.InnerException.Message != $"Branch \"{branchName}\" does not exist in the {repo.Id} repository.")
